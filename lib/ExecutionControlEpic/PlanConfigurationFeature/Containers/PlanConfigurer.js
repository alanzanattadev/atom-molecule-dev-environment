...
import {selectPlansSchemaReducer, selectPackagesReducer} from "../../../GlobalSystem/Selectors";
...
export function mapDispatchToProps(dispatch: (action: any) => any, ownProps: {toolId: string, toolIconUri: string, toolName: string}): {onAddPlan: (plan: {name: string, config: mixed, stager: Stager}) => void} {
  return {
    onAddPlan: (plan: {name: string, config: mixed, stager: Stager, packageInfos: PackageInfos}) => {
      dispatch(addPlanConfig(
        plan.name,
