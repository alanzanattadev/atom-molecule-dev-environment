...
describe('PlanConfigConditionalGroup', () => {
  it('should display the expression', () => {
    let subject = mount(<PlanConfigConditionalGroup expression={{
      type: 'string',
      default: 'ubuntu',
      description: 'distrib'
    }} cases={{
    ...
    expect(subject.find(PlanConfigTextInputField).at(0).prop('value')).toBe('ubuntu');
  });
  ...
  it('should display the case that matches with the expression', () => {
    ...
    expect(subject.find(PlanConfigNumberInputField).at(0).prop('value')).toBe(14);
  });
