...
export default class PlanConfigurer extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <div style={{maxWidth: '500px', padding: '10px 20px', display: 'flex', flexDirection: 'column', alignItems: 'stretch', flex: '1'}}>
        <PlanConfigTextInputField label="name" value={this.state.name} onChange={value => this.setState(fromJS(this.state).set('name', value).toJS())}/>
        <h1 className="text-color-highlight" style={{fontSize: '16px'}}>Config</h1>
        <PlanConfigPart value={this.state.config} onChange={value => this.setState(fromJS(this.state).set('config', value).toJS())} {...this.props.config}/>
        ...
        <span style={{alignSelf: 'flex-end'}}>
          <AddButton onClick={() => this.props.onAddPlan({
