...
describe("PlanConfigList", () => {
  ...
  it("should display the values", () => {
    expect(subject.find(PlanConfigPart).length).toBe(6); // nested parts
    expect(subject.find(PlanConfigPart).at(0).prop("value")).toEqual({
      name: "NODE_PLAN",
      value: "web",
    });
    expect(subject.find(PlanConfigPart).at(0).prop("type")).toBe("object");
    expect(subject.find(PlanConfigPart).at(0).prop("schemas")).toEqual({
      name: {
        type: "string",
        default: "",
        placeholder: "name",
