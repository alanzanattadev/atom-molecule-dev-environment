...
describe('PlanConfigPart', () => {
  it('should display a PlanConfigSelectInputField when type is enum', () => {
    let subject = mount(<PlanConfigPart type="enum" default="on-failure" value="on-failure" enum={[{
      value: 'no',
      description: 'No'
    }, {
      value: 'always',
    ...
    expect(subject.find(PlanConfigSelectInputField).prop('options')).toBeDefined();
    expect(subject.find(PlanConfigSelectInputField).prop('default')).toBe('on-failure');
    expect(subject.find(PlanConfigSelectInputField).prop('value')).toBe('on-failure');
  });

  it('should display a PlanConfigGroup when type is object', () => {
    let subject = mount(<PlanConfigPart type="object" schemas={{
      image: { type: 'string' },
      command: { type: 'string' },
  ...
  it('should display a PlanConfigList when type is array', () => {
    let values = ["coucou"];
    let subject = mount(<PlanConfigPart type="array" items={{type: "string"}} default="" value={values}/>);

    expect(subject.find(PlanConfigList).length).toBe(1);
    expect(subject.find(PlanConfigList).at(0).prop('items')).toEqual({type: 'string'});
    expect(subject.find(PlanConfigList).at(0).prop('default')).toBe("");
    expect(subject.find(PlanConfigList).at(0).prop('values')).toBe(values);
  });
  ...
  it('should display a PlanConfigTextInputField when type is string', () => {
    ...
    expect(subject.find(PlanConfigTextInputField).prop('value')).toBe('my value');
  });
  ...
  it('should display a PlanConfigNumberInputField when type is number', () => {
    ...
    expect(subject.find(PlanConfigNumberInputField).at(0).prop('value')).toBe(3);
  });
  ...
  it('should display a PlanConfigConditionalGroup when type is conditional', () => {
    ...
    subject.find(PlanConfigConditionalGroup).at(0).prop('onChange')({expressionValue: 'fedora', caseValue: 23});

    expect(subject.find(PlanConfigConditionalGroup).length).toBe(1);
    expect(subject.find(PlanConfigConditionalGroup).at(0).prop('value')).toEqual({expressionValue: 'ubuntu', caseValue: 14});
