...
describe("PlanConfigPart", () => {
  ...
  it(
    "should display a PlanConfigCheckBoxInputField when type is boolean",
    () => {
      let subject = mount(<PlanConfigPart type="boolean" value={true} />);

      expect(subject.find(PlanConfigCheckboxInputField).length).toBe(1);
      expect(subject.find(PlanConfigCheckboxInputField).prop("value")).toBe(
        true
      );
    }
  );
  ...
  it(
    "should display a PlanConfigConditionalGroup when type is conditional",
    () => {
      let spy = jest.fn();
      let subject = mount(
        <PlanConfigPart
