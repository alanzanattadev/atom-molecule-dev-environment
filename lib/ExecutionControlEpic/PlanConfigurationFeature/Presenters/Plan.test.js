...
describe("Plan", () => {
  it("should display the plan name", () => {
    ...
    expect(
      subject.findWhere(
        comp => comp.type() == "span" && comp.text() == "build",
      ).length,
    ).toBe(1);
  });

  it(
    "should display a context menu with Pin and Remove when pinnable is passed",
    () => {
  ...
  it(
    "should display a context menu with Unpin and Remove when pinnable is not passed",
    () => {
  ...
  it(
    "should call onPin when Pin in context menu is clicked with pinnable",
    () => {
      let spy = jest.fn();
  ...
  it(
    "should call onRemove when Remove in context menu is clickedwith pinnable",
    () => {
      let spy = jest.fn();
      let subject = mount(
  ...
  it(
    "should call onUnpin when Un-pin in context menu is clicked without pinnable",
    () => {
      let spy = jest.fn();
  ...
  it(
    "should call onRemove when Remove in context menu is clicked without pinnable",
    () => {
      let spy = jest.fn();
