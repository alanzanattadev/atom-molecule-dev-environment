...
export function renderToolPlans(tool: DevToolInfos) {
  ReactDOM.render((
    <Provider store={store}>
      <StyleRoot>
        <ToolPlansPanel toolId={tool.id} toolName={tool.name} toolIconUri={tool.iconUri} onClose={() => toolPlansPanel.hide()} onPinned={() => pinnedPlansPanel.show()}/>
      </StyleRoot>
    </Provider>
  ), toolPlans);
};
...
export function renderToolConfigurer(rootElem, tool: DevToolInfos) {
  ReactDOM.render((
    <Provider store={store}>
      <StyleRoot>
        <PlanConfigPanel toolId={tool.id} toolName={tool.name} toolIconUri={tool.iconUri} onClose={() => sidePanelPanel.hide()}/>
      </StyleRoot>
    </Provider>
  ), rootElem);
};
