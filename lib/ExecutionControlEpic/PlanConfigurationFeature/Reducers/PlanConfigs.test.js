...
describe('PlanConfigs', () => {
  describe('ADD_TARGET_CONFIGURATION', () => {
    it('should add a plan configuration', () => {
      let state = [];
      let action: AddPlanConfigAction = addPlanConfig("ls", {
        id: "toolid",
        name: "shell",
        iconUri: 'atom://myplugin/icon.png',
      }, "ls -l", {type: 'integrated'})
  ...
  describe('PIN_TARGET_CONFIGURATION', () => {
    it('should pin plan config', () => {
      ...
      let action: PinPlanConfigAction = pinPlanConfig(Object.assign({}, planConfig, {state: null}));
      let subject = PlanConfigs(state, action);

      expect(subject).toEqual([Object.assign({}, planConfig, {pinned: true}), planConfig2]);
    });
  });
  ...
  describe('UNPIN_TARGET_CONFIGURATION', () => {
    it('should unpin plan config', () => {
      ...
      let state = [planConfig, planConfig2];
      let action: UnpinPlanConfigAction = unpinPlanConfig(Object.assign({}, planConfig, {state: null}));
      let subject = PlanConfigs(state, action);

      expect(subject).toEqual([Object.assign({}, planConfig, {pinned: false}), planConfig2]);
    });
  });
});
