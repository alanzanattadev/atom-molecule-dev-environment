...
describe("PlanConfigs", () => {
  ...
  describe("REMOVE_GENERATED_PLANS", () => {
    it("should remove generated plans", () => {
      const planConfig3 = { ...planConfig, autoGenerated: true };
      const planConfig4 = { ...planConfig2, autoGenerated: true };

      let state = Map({
        toolid: Map({ "0": planConfig, "1": planConfig3, "2": planConfig4 }),
      });
      let action: RemoveGeneratedPlansAction = removeGeneratedPlans(
        planConfig.tool,
        planConfig.packageInfo.path,
      );
      let subject = PlanConfigs(state, action);
