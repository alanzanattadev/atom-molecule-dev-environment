  ...
  return {
    tools: selectDevtools(selectDevtoolsReducer(state)).map(devtool => {
      ...
      return fromJS(devtool)
        ...
        .set("warnings", diagnostics.filter(d => d.type == "warning").count())
        .set("errors", diagnostics.filter(d => d.type == "error").count())
        .set("successes", diagnostics.filter(d => d.type == "success").count())
        .toJS();
    })
  };
}
...
export function mapDispatchToProps(
  ...
  return {
    ...
    onToolLogsClick: (devtool: DevToolWithDiagnosticsType) => {
      dispatch(openDiagnosticsTraveller(devtool.id, devtool.name, devtool.defaultDiagnosticsMode));
    },
    onToolDiagnosticsClick: (
      devtool: DevToolWithDiagnosticsType,
      type: DiagnosticType
