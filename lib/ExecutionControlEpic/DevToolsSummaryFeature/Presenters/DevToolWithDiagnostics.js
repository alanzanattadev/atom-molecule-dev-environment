...
function getEntityWidth(props: Props) {
  return props.devtoolSize + 10 + countCircles(props) / 2 * props.circleSize;
}
...
function getSingleCircleStyles(props: Props) {
  return [
    css`
      ...
      left: ${getEntityWidth(props) / 2 - props.circleSize / 2}px;
      right: 0;
      transform: ${props.hovered ? "translate3d(-5px, -22px, 0)" : ""};
    `,
  ];
}
  ...
  return [
    css`
      ...
      transform: ${props.hovered
        ? "translate3d(-10px, -22px, 0px)"
        : "translate3d(0px, 0px, 0px)"};
    `,
    css`
      top: -${props.circleSize}px;
      right: ${getEntityWidth(props) / 2 - props.circleSize - 2}px;
      transform: ${props.hovered
        ? "translate3d(10px, -22px, 0)"
        : "translate3d(0px, 0px, 0px)"};
    `,
  ];
}
...
function getTripleCirclesStyles(props: Props) {
  return [
    css`
      top: -${props.circleSize}px;
      left: ${getEntityWidth(props) / 2 - props.circleSize * 1.5 - 5}px;
      transform: ${props =>
        props.hovered
          ? "translate3d(-15px, -16px, 0)"
          : "translate3d(0px, 0px, 0px)"};
    `,
    css`
      ...
      left: ${getEntityWidth(props) / 2 - props.circleSize / 2}px;
      transform: ${props.hovered
        ? "translate3d(-5px, -22px, 0)"
        : "translate3d(0px, 0px, 0px)"};
    `,
    css`
      top: -${props.circleSize}px;
      right: ${getEntityWidth(props) / 2 - props.circleSize * 1.5 - 5}px;
      transform: ${props.hovered
        ? "translate3d(15px, -16px, 0)"
        : "translate3d(0px, 0px, 0px)"};
    `,
...
function getFourCirclesStyles(props: Props) {
  return [
    css`
      top: -${props.circleSize - 10}px;
      left: ${getEntityWidth(props) / 2 - props.circleSize * 1.8}px;
      transform: ${props.hovered
    ...
    css`
      top: -${props.circleSize - 10}px;
      right: ${getEntityWidth(props) / 2 - props.circleSize * 1.8}px;
      transform: ${props.hovered
        ? "translate3d(16px, -10px, 0)"
        : "translate3d(0px, 0px, 0px)"};
    `,
...
const DevtoolIconPositionner = styled.div`
  transform: ${props =>
    props.hovered ? `translate3d(0, -22px, 0)` : `translate3d(0, 0, 0)`};
  justify-content: center;
  align-items: center;
  ...
  render() {
    return (
      <DevtoolWithDiagnosticsBox hovered={this.state.hovered} {...this.props}>
        {[
          { type: 2, number: this.props.warnings },
          { type: 1, number: this.props.errors },
          { type: 5, number: this.props.successes },
          { type: 3, number: this.props.infos },
        ]
...
DevToolWithDiagnostics.defaultProps = {
  ...
  onDiagnosticClick(type: DiagnosticSeverity) {},
  onSettingsClick() {},
  onLogsClick() {},
};
...
type DefaultProps = {
  ...
  circleSize: 20,
  showColor: false,
  onDiagnosticClick(type: DiagnosticSeverity): void,
      ...
      notifier:
        notifier == null
          ...
          : Rx.Observable.create(function subscribe(observer) {
              ...
              notifier.addListener("data", handler);
              return function unsubscribe() {
                notifier.removeListener("data", handler);
              };
            }),
    }),
  ),
