...
export class DevToolWithDiagnostics extends React.Component<DefaultProps, Props, State> {
  ...
  getTripleCirclesStyles() {
    return [{
      ...
      left: `${(this.getEntityWidth()) / 2 - this.props.circleSize * 1.5 - 5}px`,
      ...
      top: `${this.props.circleSize + 20}px`,
      right: `${(this.getEntityWidth()) / 2 - this.props.circleSize * 1.5 - 5}px`,
      transform: this.state.hovered ? 'translate3d(15px, -16px, 0)' : '',
    }];
  }
  ...
  getEntityWidth() {
    return (this.props.devtoolSize + 20) + ((this.countCircles(this.props) / 2) * this.props.circleSize);
  }
