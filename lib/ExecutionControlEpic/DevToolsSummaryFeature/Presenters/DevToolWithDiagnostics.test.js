...
describe("DevToolWithDiagnostics", () => {
  it("should only display tool icon when no diagnostics exists", () => {
    let subject = shallow(<DevToolWithDiagnostics iconUri="atom://haha.png" planColor="#836BEB" showColor legend="server" index={5}/>);
    ...
    expect(subject.find(DevTool).at(0).prop("color")).toBe("#836BEB");
    expect(subject.find(DevTool).at(0).prop("showColor")).toBe(true);
    expect(subject.find(DevTool).at(0).prop("legend")).toBe("server");
    expect(subject.find(DevTool).at(0).prop("index")).toBe(5);
  });

  it(
    "should display tool icon with one yellow circle with number if only warnings exist",
    () => {
  ...
  it(
    "should display tool icon with with one yellow / red circle if errors and warnings exist",
    () => {
  ...
  it(
    "should display tool icon with one blue / red circle if errors and infos exist",
    () => {
      let subject = shallow(
  ...
  it(
    "should display tool icon with blue / red / yellow / green circle if every diagnostic exist",
    () => {
      let subject = shallow(
  ...
  it(
    "should call onDiagnosticClick with diagnostic type on diagnostic circle click",
    () => {
      let spy = jest.fn();
      let subject = shallow(
