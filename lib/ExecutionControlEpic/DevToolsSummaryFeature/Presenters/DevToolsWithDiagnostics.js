...
export default class DevToolsWithDiagnostics
  ...
  componentDidMount() {
    const addEventListener = eventName => (handler) => {
      if (global.atom) {
        atom.workspace.observePanes(pane => {
          atom.views.getView(pane).addEventListener(eventName, handler)
        })
      } else {
    ...
    const removeEventListener = eventName => (handler) => {
      if (global.atom) {
        atom.workspace.observePanes(pane => {
          atom.views.getView(pane).removeEventListener(eventName, handler)
        })
      } else {
    ...
    this.unsubscribeKeyObs = Rx.Observable
      .combineLatest(
        Rx.Observable.merge(
          Rx.Observable
            .fromEventPattern(addEventListener('keyup'), removeEventListener('keyup'))
            .filter(e => e.key == "Alt")
            .mapTo(false),
          Rx.Observable
            .fromEventPattern(addEventListener('keydown'), removeEventListener('keydown'))
            .filter(e => e.key == "Alt")
            .mapTo(true)
        ),
        Rx.Observable.merge(
