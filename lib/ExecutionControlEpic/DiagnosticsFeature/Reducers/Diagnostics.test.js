...
import FakeDiagnostics, {dockerWeb1Error1, dockerWeb1Error2, dockerWeb2Warning1, dockerDBInfo1} from "../Fake/Data/FakeDiagnostics";

describe('Diagnostics Reducer', () => {
  describe('ADD_DIAGNOSTIC_FOR_TASK', () => {
    it('should add a diagnostic with the task as attribut', () => {
      let state: DiagnosticsReducer = [FakeDiagnostics[0], FakeDiagnostics[1]];
      let action: AddDiagnosticForTaskAction = addDiagnosticForTask(FakeDiagnostics[2]);
      let subject = Diagnostics(state, action);

      expect(subject).toEqual([FakeDiagnostics[0], FakeDiagnostics[1], FakeDiagnostics[2]]);
    });
  });

  describe('REMOVE_DIAGNOSTICS_OF_TASK', () => {
    it('should remove every task of given task id', () => {
      let state: DiagnosticsReducer = [dockerDBInfo1, dockerWeb1Error1, dockerWeb1Error2, dockerWeb2Warning1];
      let action: RemoveDiagnosticsForTaskAction = removeDiagnosticsForTask(dockerWeb1Error1.task);
      let subject = Diagnostics(state, action);
