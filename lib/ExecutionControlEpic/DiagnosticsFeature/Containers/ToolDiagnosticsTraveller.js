...
import {selectDiagnosticsReducer, selectPlansReducer, selectTasksReducer} from "../../../GlobalSystem/Selectors";
    ...
    return (
      <ToolDiagnostics
        onTaskClick={(task: Task) => {
          let index;
          if (
            (index = this.state.selectedTasks.findIndex(id => id == task.id)) !=
            -1
          ) {
            this.setState({
...
export function mapDispatchToProps(dispatch: (action: any) => any): {onJumpTo(location: DiagnosticLocation): void} {
  return {
