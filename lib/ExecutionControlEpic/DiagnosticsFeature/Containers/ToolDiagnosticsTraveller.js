...
import {selectDiagnosticsReducer, selectTasksReducer, selectTargetsReducer} from "../../../GlobalSystem/Selectors";
...
import {selectTargetsOfTool} from "../../TargetConfigurationFeature/Selectors/TargetConfigs";
...
export class DiagnosticsStatefulSelector extends React.Component<DefaultProps, Props, ComponentState> {
  ...
  getDiagnosticsOfTasks() {
    return this.state.selectedTasks
            .map(taskId => selectDiagnosticsOfTask(selectDiagnosticsReducer(this.props.state), taskId))
  ...
  getTargetsWithTasks() {
    ...
    return selectTargetsOfTool(selectTargetsReducer(this.props.state), this.props.toolId)
            .map(target => ({
              target: target,
              tasks: tasksOfTool
                      .filter(task => areSameTargets(task.target, target))
  ...
  render() {
    return (
      <ToolDiagnostics
        ...
        onTargetClick={(target: TargetConfig) => {}}
        diagnostics={this.getDiagnosticsOfTasks()}
        targetsWithTasks={this.getTargetsWithTasks()}
