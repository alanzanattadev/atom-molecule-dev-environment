...
export function selectDiagnosticsOfStep(diagnostics: Array<Diagnostic>, step: number) {
  return diagnostics.filter(diagnostic => diagnostic.step == step);
}

export function selectLastDiagnostics(diagnostics: Array<Diagnostic>) {
  const max = diagnostics.map(d => d.step).reduce((red, value) => value > red ? value : red, 0);
