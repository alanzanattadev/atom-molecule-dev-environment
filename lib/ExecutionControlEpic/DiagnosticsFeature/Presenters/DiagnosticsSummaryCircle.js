    ...
    return (
      <div style={{
      ...
      }} onClick={this.props.onClick} onMouseEnter={() => this.setState({hovered: true})} onMouseLeave={() => this.setState({hovered: false})}>
        {
          this.state.hovered ?
            (
              <Image src={'atom://molecule-dev-environment/.storybook/public/' + this.getIconForType(this.props.type)} fallbackImage={this.getIconForType(this.props.type)} style={{color: 'white', height: `${parseInt(this.props.size) / 2}px`, width: `${parseInt(this.props.size) / 2}px`}}/>
            ) :
            (
