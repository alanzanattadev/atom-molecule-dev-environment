...
export default class DiagnosticsSummaryCircle extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <div className={{className: getColorForDiagnosticType(this.props.type)}}
        style={{
      ...
      }} onClick={this.props.onClick} onMouseEnter={() => this.setState({hovered: true})} onMouseLeave={() => this.setState({hovered: false})}>
        {
          this.state.hovered ?
            (
              <Image src={'atom://molecule-dev-environment/.storybook/public/' + this.getIconForType(this.props.type)} fallbackImage={this.getIconForType(this.props.type)} className="text-color-highlight" style={{height: `${parseInt(this.props.size) / 2}px`, width: `${parseInt(this.props.size) / 2}px`}}/>
            ) :
            (
              <span className="text-color-highlight" style={{fontSize: `${parseInt(this.props.size) / 2}px`}}>{this.props.number}</span>
            )
        }
      </div>
    )
