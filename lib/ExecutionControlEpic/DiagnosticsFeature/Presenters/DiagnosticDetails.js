...
export default class DiagnosticDetails extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    if (typeof this.props.message == 'object' && this.props.message.html) {
      return (
        <div style={{maxWidth: '800px', fontSize: '15px', overflow: 'auto'}} dangerouslySetInnerHTML={{__html: this.props.message.text}}>

        </div>
      );
    } else if (typeof this.props.message == 'object' && this.props.message.data) {
      return (
        <div style={{maxWidth: '800px', fontSize: '15px', overflow: 'auto'}}>
    ...
    } else if (typeof this.props.message == 'function' || isClassComponent(this.props.message)) {
