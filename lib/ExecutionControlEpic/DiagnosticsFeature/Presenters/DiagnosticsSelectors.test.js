...
describe('DiagnosticsSelectors', () => {
  it('should display warnings and errors when there is no infos and successes', () => {
    ...
    let subject = shallow(<DiagnosticsSelectors diagnostics={diagnostics} diagnosticType="warning"/>);

    expect(subject.find(DiagnosticsSelector).length).toBe(2);
    expect(subject.find(DiagnosticsSelector).at(0).prop('type')).toBe('warning');
  ...
  it('should display infos and successes when there is no warnings and errors', () => {
    ...
    expect(subject.find(DiagnosticsSelector).length).toBe(2);
    expect(subject.find(DiagnosticsSelector).at(0).prop('type')).toBe('success');
  ...
  it('should call onSelected with type and index when a back arrow is clicked', () => {
    ...
    let spy = jest.fn();
    let subject = shallow(<DiagnosticsSelectors diagnostics={diagnostics} onSelected={spy} diagnosticIndex={1} diagnosticType="success"/>);
  ...
  it('should call onSelected with type and index when a forward arrow is clicked', () => {
    ...
    let spy = jest.fn();
    let subject = shallow(<DiagnosticsSelectors diagnostics={diagnostics} onSelected={spy} diagnosticIndex={0} diagnosticType="success"/>);
