...
export default class DiagnosticsTraveller extends React.Component<DefaultProps, Props, State> {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.getSelectedDiagnostics(this.props.diagnostics, this.state.traveller.type).length >= this.state.traveller.index)
      this.setState({
        traveller: this.getDefaultDiagnostic()
      });
  }
  ...
  getDefaultDiagnostic() {
    if (this.getSelectedDiagnostics(this.props.diagnostics, 'error').length > 0) {
      return ({
        index: 0,
        type: 'error'
      });
    } else if (this.getSelectedDiagnostics(this.props.diagnostics, 'warning').length > 0) {
      return ({
        index: 0,
        type: 'warning'
      });
    } else if (this.getSelectedDiagnostics(this.props.diagnostics, 'info').length > 0) {
      return ({
        index: 0,
        type: 'info'
      });
    } else if (this.getSelectedDiagnostics(this.props.diagnostics, 'success').length > 0) {
      return ({
        index: 0,
        type: 'success'
      });
