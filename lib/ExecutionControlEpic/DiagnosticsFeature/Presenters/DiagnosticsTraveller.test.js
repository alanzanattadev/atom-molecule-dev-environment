...
describe('DiagnosticsTraveller', () => {
  it('should display diagnostics selectors', () => {
    let diagnostics: Array<Diagnostic> = [
      {type: 'warning', message: 'a message'},
      {type: 'success', message: 'a message'}
    ];
    let subject = shallow(<DiagnosticsTraveller diagnostics={diagnostics}/>);

    expect(subject.find(DiagnosticsSelectors).length).toBe(1);
    expect(subject.find(DiagnosticsSelectors).at(0).prop('diagnosticIndex')).toBe(0)
    expect(subject.find(DiagnosticsSelectors).at(0).prop('diagnosticType')).toBe('error')
  });
  ...
  it('should display warnings details when warning selector is selected', () => {
    let diagnostics: Array<Diagnostic> = [
      {type: 'warning', message: 'a message'},
      {type: 'success', message: 'a message'}
    ];
  ...
  it('should display success details when infos selector is selected', () => {
    let diagnostics: Array<Diagnostic> = [
      {type: 'warning', message: 'a message'},
      {type: 'success', message: 'a message of success'}
    ];
    let subject = mount(<DiagnosticsTraveller diagnostics={diagnostics}/>);

    subject.find(DiagnosticsSelectors).at(0).prop('onSelected')('success', 0);

    expect(subject.find(DiagnosticDetails).at(0).prop('message')).toBe('a message of success');
  });

  it('should display second success details when onForward is called for success selectors', () => {
    let diagnostics: Array<Diagnostic> = [
      {type: 'warning', message: 'a message'},
      {type: 'success', message: 'a message of success'},
      {type: 'success', message: 'a message of success 2'},
    ];
    let subject = mount(<DiagnosticsTraveller diagnostics={diagnostics}/>);

    subject.find(DiagnosticsSelectors).at(0).prop('onSelected')('success', 1);

    expect(subject.find(DiagnosticDetails).at(0).prop('message')).toBe('a message of success 2');
  });

  it('should display every details when log mode is selected', () => {
    let diagnostics: Array<Diagnostic> = [
      {type: 'warning', message: 'a message'},
      {type: 'success', message: 'a message haha'},
      {type: 'success', message: 'a message haha'},
      {type: 'success', message: 'a message'},
    ];
    let subject = mount(<DiagnosticsTraveller diagnostics={diagnostics}/>);
    ...
    expect(subject.find(DiagnosticDetailsFlow).at(0).prop('diagnostics').length).toBe(2);
  });
  ...
  it('should filter with the search', () => {
    ...
    expect(subject.find(DiagnosticsSelectors).at(0).prop('diagnostics')).toEqual([]);
  });
});
