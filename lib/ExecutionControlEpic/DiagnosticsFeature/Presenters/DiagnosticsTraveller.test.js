...
import type { Diagnostic, DiagnosticMode } from "../Types/types.js.flow";
...
describe("DiagnosticsTraveller", () => {
  ...
  it("should display every details when log mode is selected", () => {
    ...
    subject.find(DiagnosticsModeSelector).at(0).prop("onClick")("logs");
  ...
  it("should filter with the search", () => {
    let diagnostics: Array<Diagnostic> = [
      ...
      {
        type: "success",
        message: { html: false, text: "an other message" },
        step: 1,
      },
      { type: "", message: null, step: 1 },
    ];
    let subject = mount(<DiagnosticsTraveller diagnostics={diagnostics} />);
