  ...
  ({
    ...
    return {
      diagnostics: diagnostics.filter(d =>
        contentState.diagnosticsTypes.includes(d.severity),
      ),
    };
  },
);
...
const ContentModeAdapter = withProps(
  ({
    ...
    onToggleDiagnosticType(type: DiagnosticSeverity) {
      if (
        contentState.diagnosticsTypes.size > 1 ||
        contentState.diagnosticsTypes.includes(type) === false
      ) {
...
}: // onJumpTo = () => {},
{
  ...
  onToggleDiagnosticType: (type: DiagnosticSeverity) => void,
    ...
    <Panel>
      <ModeBox>
        <DiagnosticsModeSelector
          ...
          terminal={currentTask && currentTask.strategy.type === "terminal"}
        />
      </ModeBox>
      <ContentBox
        className={`diagnostic-content ${mode === "terminal"
          ? "terminal"
          : ""}`}
      >
        {mode === "terminal" ? (
