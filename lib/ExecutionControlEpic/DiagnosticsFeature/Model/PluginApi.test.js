...
describe("Diagnostics Plugins API", () => {
  describe("provideDiagnostics", () => {
    it("should dispatch one ADD_PUBLISHED_DIAGNOSTICS_FOR_TASK", () => {
      let spy = jest.fn();
      let store = {
        dispatch: spy,
      ...
      expect(
        spy.mock.calls.find(
          call =>
            call[0].type == "ADD_PUBLISHED_DIAGNOSTICS_FOR_TASK" &&
            call[0].payload.publishedDiagnostics.diagnostics[0].task ==
              "taskid",
        ),
      ).toBeDefined();
    });
