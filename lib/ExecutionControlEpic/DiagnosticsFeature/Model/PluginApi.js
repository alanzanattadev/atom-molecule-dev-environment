...
export function provideDiagnostics(taskID: string, store: {dispatch: (action: any) => void, getState: () => any}): (diagnostics: Array<Diagnostic>) => void {
  return function(diagnostics: Array<Diagnostic>): void {
    ...
    store.dispatch(addDiagnosticsForTask(diagnostics.map(diagnostic => Object.assign(diagnostic, {task: taskID, step: task.step}))));
  };
};
...
export default function(taskID: string, store: {dispatch: (action: any) => void, getState: () => any}): TaskAPI {
  return {
