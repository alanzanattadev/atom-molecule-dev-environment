...
export default {
  ...
  getStrategyForPlan(plan: PlanConfig, helperApi: HelperApi) {
    ...
    return {
      strategy: {
        type: "shell",
        command: `${binaryPath} --json ${plan.config.config ? `-o` : ""} ${plan.config.configfile !== "-- Not Selected --" ? `--config ${plan.config.configfile}` : ""} --useStderr --silent ${plan.config.watch ? "--watch" : ""} --outputFile ${getOutputFilePath(plan.name, helperApi)}`,
