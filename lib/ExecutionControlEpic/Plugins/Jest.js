...
export default {
  ...
  getStrategyForPlan(plan: PlanConfig, helperApi: HelperApi) {
    ...
    return {
      strategy: {
        type: "shell",
        command: `${binaryPath} --json ${plan.config.config ? `-o` : ""} ${plan.packageInfos.path !== "package.json" ? `--config ${plan.packageInfos.path}` : ""} --useStderr --silent ${plan.config.watch ? "--watch" : ""} --outputFile ${getOutputFilePath(plan.name, helperApi)}`,
  ...
  isPackage: (packagePath: string, dirname: string) =>
