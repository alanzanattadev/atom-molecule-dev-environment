...
export default {
  infos: {
    name: "gulp",
    iconUri: "atom://molecule-dev-environment/.storybook/public/devtool-icon-gulp.png",
  },
  configSchema: {
    type: "object",
    schemas: {
  ...
  getStrategyForPlan(plan: PlanConfig) {
    ...
    if (plan.config.binary.expressionValue == "local")
      binaryPath = `${path.join(path.dirname(plan.packageInfos.path), "node_modules", ".bin", "gulp")}`;
