...
export default {
  ...
  getStrategyForPlan(plan: PlanConfig) {
    ...
    if (plan.config.binary.expressionValue === "local") {
      binaryPath = `${path.join(path.dirname(plan.packageInfos.path), "node_modules", ".bin", "nightwatch")}`;
    } else binaryPath = "nightwatch";
    const cmd = `${binaryPath} ${sources} --reporter ${path.join(__dirname, "html-reporter.js")} --env ${plan.config.env}`;
    return {
      strategy: {
        type: "shell",
        command: cmd,
          ...
          fs.readFile("/tmp/report-nightwatch.json", "utf8", function(
            ...
            helperAPI.json.parseAsync(data.toString()).then(json => {
              let getMessageForDiagnostic = (m): string => {
                const type = typeof m.failure === "string"
                  ? "error"
                  : "success";
                if (type === "error") {
  ...
  isPackage: (packageName: string, dirname: string) => {
    if (path.basename(packageName) === "package.json") {
      const check = new File(
        `${packageName.substr(0, packageName.lastIndexOf("/"))}/node_modules/nightwatch`,
      );
      return check.exists();
    } else {
      return false;
