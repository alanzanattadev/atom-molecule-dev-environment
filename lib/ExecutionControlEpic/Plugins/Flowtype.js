...
export default {
  ...
  getStrategyForPlan(plan: PlanConfig) {
    ...
    if (plan.config.conf.expressionValue == "server") {
      cmd = `${binaryPath} ${plan.config.conf.caseValue.serverconfig.expressionValue ==
        "start"
        ? plan.config.conf.caseValue.serverconfig.caseValue
            ? "--quiet --json --strip-root"
            : "start"
        : "stop"}`;
    } else {
      cmd = `${binaryPath} status ${plan.config.conf.caseValue.allerrors
        ? "--show-all-errors "
        : ""}--no-auto-start --json --strip-root`;
    }
    return {
      strategy: {
        type: "shell",
            ...
            .then(json => {
              ...
              taskAPI.addDiagnostics(
                (json.errors || []).map(err => ({
                  ...
                  message: {
                    text: err.message
                      .map(m => getMessageForDiagnostic(m))
                      .join(""),
                    html: false
                  },
                  date: moment().unix()
                })).concat((json.warnings || []).map(err => ({
                  type: "error",
                  message: {
                    text: err.message
                      .map(m => getMessageForDiagnostic(m))
