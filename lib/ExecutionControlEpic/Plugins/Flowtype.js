...
export default {
  ...
  getStrategyForTarget(target: TargetConfig) {
    ...
    if (target.config.binary.expressionValue == 'local')
      binaryPath = `${path.join(path.dirname(target.packageInfos.path), 'node_modules', '.bin', 'flow')}`;
    else
      binaryPath = 'flow';
    if (target.config.conf.expressionValue == 'server') {
      cmd = `${binaryPath} ${(target.config.conf.caseValue.serverconfig.expressionValue == 'start') ?
      (target.config.conf.caseValue.serverconfig.caseValue ? '--quiet --json --strip-root' : 'start') : 'stop'}`;
    } else {
      cmd = `${binaryPath} status ${target.config.conf.caseValue.allerrors ? '--show-all-errors ' : ''}--no-auto-start --json --strip-root`;
    }
    return {
      strategy: {
        type: 'shell',
