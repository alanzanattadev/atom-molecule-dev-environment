...
import type { Diagnostic } from "../../DiagnosticsFeature/Types/types.js.flow";
...
describe("DevtoolLoadingHandler", () => {
  describe("provideDevtool", () => {
    it("should dispatch an ADD_DEVTOOL action", () => {
      ...
      let devtool: ProvidedDevTool = {
        ...
        getStrategyForPlan(plan) {
          return { strategy: { type: "shell" }, controller: {} };
        }
      };
      let subject = provideDevtool(store, new DevToolsController())(devtool);
    ...
    it("should dispatch an ADD_TARGET_CONFIGURATION_SCHEMA action", () => {
      ...
      expect(
        spy.mock.calls.find(
          call => call[0].type == "ADD_TARGET_CONFIGURATION_SCHEMA"
        )
      ).toBeDefined();
    });
  });
