'use babel';
// @flow

import React from 'react';
import { Record } from 'immutable';
import { withState, lifecycle, withProps } from 'recompose';
import Rx from 'rxjs';

function execStep(step, answers, ...data) {
  console.log('Executing step');
  if (typeof step == 'function') {
    const result = step(answers, ...data);
    if (result != null && typeof result == 'object' && result.then) {
      return Rx.Observable.fromPromise(result);
    } else if (result instanceof Rx.Observable) {
      return result;
    } else {
      return Rx.Observable.create(obs => {
        obs.next(result);
        obs.complete();
      });
    }
  } else {
    return Rx.Observable.create(obs => {
      obs.next(step);
      obs.complete();
    });
  }
}

export default (questions, onAnswer) =>
  Component => withState('formState', 'setFormState', () => {
    const Answers = Record(
      questions
        .map(q => q.name)
        .reduce((red, value) => Object.assign({}, red, { [value]: null }), {})
    );
    const ReducerState = Record({
      index: 0,
      step: 'when',
      answers: null,
      questions: null,
      prevState: null,
      loaded: null
    });
    return new ReducerState({
      questions: questions,
      answers: new Answers()
    });
  })(
    lifecycle({
      testQuestion() {
        console.log(
          'when => ',
          this.props.formState,
          this.props.formState.questions[this.props.formState.index]
        );
        execStep(
          this.props.formState.questions[this.props.formState.index].when ||
            true,
          this.props.formState.answers
        )
          .toPromise()
          .then(
            response => {
              if (response) {
                console.log('Asking question');
                this.props.setFormState(current =>
                  current.set('step', 'loader'));
              } else {
                console.log('Question skipped');
                this.props.setFormState(current =>
                  current.set('index', current.index + 1));
              }
            },
            () => {}
          );
      },
      loadData() {
        console.log(
          'loader => ',
          this.props.formState.index,
          this.props.formState.questions[this.props.formState.index]
        );
        execStep(
          this.props.formState.questions[this.props.formState.index].loader ||
            null,
          this.props.formState.answers
        )
          .toPromise()
          .then(
            response => {
              console.log('loaded with success');
              this.props.setFormState(current =>
                current.set('step', 'ask').set('loaded', response));
            },
            err => {
              console.log('Error while loading', err);
            }
          );
      },
      componentDidMount() {
        console.log('Mounted, testing', this.props);
        if (this.props.formState.step == 'when') {
          this.testQuestion();
        }
      },
      componentDidUpdate(prevProps) {
        console.log('Updated, testing', this.props);
        if (this.props.formState.index == this.props.formState.questions.length)
          onAnswer(this.props.formState.answers);
        else if (
          (prevProps.formState.step != 'when' ||
            prevProps.formState.index != this.props.formState.index) &&
          this.props.formState.step == 'when'
        ) {
          this.testQuestion();
        } else if (
          prevProps.formState.step == 'when' &&
          this.props.formState.step == 'loader'
        ) {
          this.loadData();
        }
      }
    })(
      withProps(props => ({
        onNext(data) {
          console.log('On Next', data);
          props.setFormState(current => current.set('step', 'validation'));
          console.log(
            'Validator => ',
            props.formState,
            props.formState.questions[props.formState.index]
          );
          execStep(
            props.formState.questions[props.formState.index].validator || true,
            Object.assign({}, props.formState.answers, {[props.formState.questions[props.formState.index].name]: data}),
            data
          )
            .toPromise()
            .then(
              (validated) => {
                console.log('Validated');
                if (validated) {
                  props.setFormState(current =>
                    current
                    .set('index', current.index + 1)
                    .setIn(
                      [
                        'answers',
                        props.formState.questions[props.formState.index].name
                      ],
                      data
                    )
                    .set('prevState', current.set('step', 'ask'))
                    .set('loaded', null)
                    .set('step', 'when'));
                } else {
                  props.setFormState(current => current.set('step', 'ask'));
                }
              },
              err => {
                console.log('Error while validating', err);
              }
            );
        },
        onPrev() {
          props.setFormState(props.formState.prevState)
        },
        onEscape() {
          onAnswer(null);
        },
        index: props.formState.index,
        step: props.formState.step,
        first: props.formState.prevState == null,
        questions: props.formState.questions.map(question => {
          let dflt = typeof question.default == 'function'
            ? question.default(props.formState.answers, props.formState.loaded)
            : question.default;
          dflt = typeof dflt == 'string'
            ? { value: dflt, description: dflt }
            : dflt;
          const message = typeof question.message == 'function'
            ? question.message(props.formState.answers, props.formState.loaded)
            : question.message;
          const choices = ((typeof question.choices == 'function'
            ? question.choices(props.formState.answers, props.formState.loaded)
            : question.choices) || []).map(
            choice =>
              typeof choice == 'string'
                ? { value: choice, description: choice }
                : choice
          );
          const accept = typeof question.accept == 'string'
            ? { description: accept, value: true }
            : accept;
          const decline = typeof question.decline == 'string'
            ? { description: decline, value: false }
            : decline;
          return {
            type: question.type,
            default: dflt,
            message: message,
            choices: choices,
            accept: accept,
            decline: decline
          };
        })
      }))(Component)
    )
  );
