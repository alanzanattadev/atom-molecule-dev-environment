...
import type {Questions, Question} from '../Types/types.js.flow';
...
export default function generateAPI(store, ) {
  const questionsPipeline = new Rx.Subject()
                                  .concatMap(({questions, id}) => Rx.Observable.defer(() => Rx.Observable.create((obs) => {
  ...
  return {
    ask(questions: Questions): rxjs$Observable<object> {
      const id = lastId + 1;
      lastId += 1;
      const obs = questionsPipeline.first(data => data.id == id).map(data => data.answers).toPromise();
