        ...
        if (languageClientConfig.connection != null) {
          ...
          editorObservable = EditorFileObservable.withLatestFrom(
            ...
            (event: MoleculeAtomEditorEvent, versions) => {
              let obj = { message: event.type, args: {} };
              if (obj.message === "didOpen") {
                obj.args = {
                  textDocument: {
                    uri: event.path,
                    languageId: event.event.textEditor.getGrammar().name,
                    version: versions[event.path] || 0,
                    text: event.event.textEditor.getText(),
                  },
                };
              } else if (obj.message === "didChange") {
                obj.args = {
                  textDocument: {
                    uri: event.path,
                    version: versions[event.path] || 0,
                  },
                  contentChanges: event.event.changes.map(change => ({
                    range: {
                      start: {
          ...
          languageClientConfig.connection.onNotification(
            "workspace/busy",
            (isBusy: boolean): void => {
              observer.next(isBusy ? busyTask(taskID) : waitingTask(taskID));
            },
          );
