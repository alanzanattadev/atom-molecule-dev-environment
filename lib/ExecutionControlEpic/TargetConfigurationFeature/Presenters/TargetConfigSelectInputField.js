...
export default class TargetConfigSelectInputField extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <select value={this.props.value} className="input-select" style={{color: '#fdfffc', backgroundColor: '#263144', borderRadius: '5px', padding: '5px', height: '40px', border: '0px', margin: '5px 5px 5px 0px'}} onChange={e => this.props.onChange(e.target.value)}>
