...
describe('TargetConfigList', () => {
  ...
  it('should display the values', () => {
    expect(subject.find(TargetConfigPart).length).toBe(2); // nested parts
    expect(subject.find(TargetConfigPart).at(1).prop('value')).toEqual({name: 'NODE_ENV', value: 'dev'});
    expect(subject.find(TargetConfigPart).at(1).prop('type')).toBe('object');
    expect(subject.find(TargetConfigPart).at(1).prop('schemas')).toEqual({
      name: {
        type: 'string',
        default: '',
        placeholder: 'name'
