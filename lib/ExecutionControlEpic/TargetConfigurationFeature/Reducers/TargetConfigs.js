...
export default function(state: TargetConfigsReducer = [], action: any): TargetConfigsReducer {
  switch(action.type) {
    case "ADD_TARGET_CONFIGURATION":
      return fromJS(state).push({name: action.payload.name, tool: action.payload.tool, config: action.payload.config, pinned: false, stager: action.payload.stager}).toJS();
