...
export function mapStateToProps(state: State, {toolId}: {toolId: string}): {targets: Array<TargetConfig & {state: TaskState}>} {
  return {
    targets: selectTargetsOfTool(selectTargetsReducer(state), toolId).map(target => fromJS(target).set('state', selectStateOfTarget(selectTasksReducer(state), target)).toJS())
  };
};

export function mapDispatchToProps(dispatch: (action: any) => any, ownProps): {onTargetPin: (target: TargetConfig) => void, onTargetClick: (target: TargetConfig) => void} {
  return {
