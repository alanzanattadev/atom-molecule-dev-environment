...
import type {ConfigSchemaPart} from "../Types/types.js";
...
export function mapDispatchToProps(dispatch: (action: any) => any, ownProps: {toolId: string, toolIconUri: string, toolName: string}): {onAddTarget: (target: {name: string, config: mixed}) => void} {
  return {
    onAddTarget: (target: {name: string, config: mixed}) => {
      dispatch(addTargetConfig(target.name, {name: ownProps.toolName, id: ownProps.toolId, iconUri: ownProps.toolIconUri}, target.config));
    }
  };
};
