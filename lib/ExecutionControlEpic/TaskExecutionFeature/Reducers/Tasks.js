  ...
  switch (action.type) {
    ...
    case "NEXT_STEP":
      return fromJS(state)
        ...
        .toJS();
    case "BUSY_TASK":
      return fromJS(state).map(
        task =>
          task.get("id") === action.payload.id ? task.set("busy", true) : task,
      );
    case "WAITING_TASK":
      return fromJS(state).map(
        task =>
          task.get("id") === action.payload.id ? task.set("busy", false) : task,
      );
    case "TERMINAL_INITIALIZED":
      return fromJS(state).map(
        task =>
          task.get("id") === action.payload.id
            ? task.set("terminal", true)
            : task,
      );
    default:
      return state;
