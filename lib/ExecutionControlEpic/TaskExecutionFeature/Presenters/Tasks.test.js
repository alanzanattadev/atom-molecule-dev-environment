...
describe("Tasks", () => {
  let tasks = [
    ...
    { id: "2", state: "running", debut: moment().subtract(2, "m").unix() },
    {
      id: "3",
      state: "running",
      debut: moment().subtract(2, "m").unix(),
      selected: true,
    },
    {
      id: "4",
      state: "failed",
      debut: moment().subtract(2, "m").unix(),
      end: moment().subtract(1, "m").unix(),
    },
    {
      id: "5",
      state: "crashed",
      debut: moment().subtract(3, "m").unix(),
      end: moment().unix(),
    },
    { id: "6", state: "running", debut: moment().subtract(3, "h").unix() },
  ];
  ...
  it("should set opacity at 0.7 if a task isn't selected", () => {
    ...
    expect(
      subject.findWhere(
        comp =>
          comp.type() == "li" &&
            comp.prop("style").opacity &&
            comp.prop("style").opacity != "1",
      ).length,
    ).toBe(5);
    expect(
      subject.findWhere(
