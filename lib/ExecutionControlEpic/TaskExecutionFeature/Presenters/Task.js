...
export default class Task extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <div style={{backgroundColor: '#21252C', padding: '10px 15px', display: 'flex', border: '1px solid #000'}} onClick={this.props.onClick}>
        ...
        <div style={{display: 'flex', marginLeft: '15px', flexDirection: 'column', alignItems: 'stretch', justifyContent: 'center'}}>
          {
            this.props.end ? (
              <div>
                <span style={{fontSize: '16px', color: 'white'}}>{moment.unix(this.props.end).fromNow()}</span>
                <hr style={{backgroundColor: 'black', height: '1px', margin: '5px 0px'}}/>
              </div>
            ) : false
          }
          <div>
            <span style={{fontSize: '16px', color: 'white'}}>{moment.unix(this.props.debut).fromNow()}</span>
          </div>
        </div>
      </div>
    )
