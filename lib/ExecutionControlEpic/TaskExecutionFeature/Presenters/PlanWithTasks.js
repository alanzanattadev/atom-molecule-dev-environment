...
export default class PlanWithTasks extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <div style={{position: 'relative', overflow: 'visible'}} onMouseEnter={() => this.setState({hovered: true})} onMouseLeave={() => this.setState({hovered: false})}>
        <Plan name={this.props.plan.name} iconUri={this.props.plan.tool.iconUri} state={this.props.plan.state} onClick={() => this.props.onPlanClick(this.props.plan)}/>
