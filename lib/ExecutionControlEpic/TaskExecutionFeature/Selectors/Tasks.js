...
export function selectPertinentTask(state: TasksReducer): Task {
  ...
  if (runningTask)
    ...
    return tasks
      .reduce(
        (red, task) => {
          if (red.get("state") == null) return task;
          else if (red.get("end") && task.get("end"))
            return red.get("end") < task.get("end") ? task : red;
          else if (task.get("end")) return task;
          else return red;
        },
        fromJS({ state: null })
      )
      .update(task => task.get("state") ? task.toJS() : null);
  }
}
...
export function selectTaskIDOfRunningPlan(
  ...
  return fromJS(state)
    .filter(
      task =>
        areSamePlans(task.get("plan").toJS(), plan) &&
...
export function selectTaskOfID(state: TasksReducer, taskId: string) {
  ...
  if (foundTask) return foundTask.toJS();
  else return null;
}

export function selectTasksOfPlan(state: TasksReducer, plan: PlanConfig): TasksReducer {
  return fromJS(state)
    .filter(
      task =>
        areSamePlans(task.get("plan").toJS(), plan)
