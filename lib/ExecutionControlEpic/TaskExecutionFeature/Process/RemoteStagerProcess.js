...
var child = exec(strategy, {
  onStdoutData(data) {process.stdout.write(data.toString() + '\n')},
  onStderrData(data) {process.stderr.write(data.toString() + '\n')},
  onExit(exitCode) {process.exit(exitCode)},
  onError(err) {process.stderr.write(JSON.stringify(err) + '\n')},
});
