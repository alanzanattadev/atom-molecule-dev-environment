...
import type { Questions } from "../../QuestionSystemFeature/Types/types.js.flow";
    ...
    return [
      {
        ...
        default: "cmd.exe /c",
        choices: [
          { value: "powershell.exe -Command", description: "powershell" },
...
export function runTask(plan: PlanConfig): RunAction {
  return (dispatch, getState) => {
    let { strategy, controller } =
      DevToolsControllerInstance.getStrategy(plan, helperApi) || {};
    ...
    const questionPromise =
      strategy.type === "shell" || strategy.type === "terminal"
    ...
    questionPromise.then(
      answers => {
        ...
        const completeStrategy: ShellStrategy = {
          ...strategy,
          env:
            strategy.env !== undefined
              ? { ...process.env, ...strategy.env }
              : process.env,
          shell: do {
            if (
              (strategy.type === "shell" || strategy.type === "terminal") &&
              answers.shell !== undefined
            )
              answers.shell;
            else if (
              strategy.type === "shell" ||
        ...
        const term =
          completeStrategy.type === "terminal"
            ? new Terminal({
                cols: 80,
                rows: 17,
              })
        ...
        let child = stager(completeStrategy, {
          onData(data) {
            if (completeStrategy.type === "terminal") {
              term.write(data);
            }
