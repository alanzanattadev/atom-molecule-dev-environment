...
export function runTask(target: TargetConfig): RunAction {
  return (dispatch) => {
    ...
    let child = IntegratedStager(strategy, {
      ...
      onExit(exitCode) { controller.onExit(exitCode, taskAPI); dispatch(crashTask(taskID, moment.unix())) },
      onError(err) { controller.onError(err, taskAPI); dispatch(crashTask(taskID, moment().unix())) },
    });
    if (child == null) {
