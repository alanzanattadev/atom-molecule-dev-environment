...
export function runTask(plan: PlanConfig): RunAction {
  return (dispatch, getState) => {
    let {strategy, controller} = DevToolsControllerInstance.getStrategy(plan) || {};
    if (strategy == null || controller == null) {
      atom.notifications.addWarning("Task not launched : bad configuration");
      return;
    }
    let taskID = generateTaskID();
    let action = addTask(taskID.toString(), fromJS(plan).remove('state').toJS(), strategy, moment().unix());
    dispatch(action);
    ...
    let child = stager(strategy, {
      onStdoutData(data) { controller.onStdoutData(data, taskAPI, helperApi) },
      onStderrData(data) { controller.onStderrData(data, taskAPI, helperApi) },
      onExit(exitCode) { controller.onExit(exitCode, taskAPI, helperApi); dispatch(crashTask(taskID.toString(), moment().unix())) },
      onError(err) { controller.onError(err, taskAPI, helperApi); dispatch(crashTask(taskID.toString(), moment().unix())) },
    });
    if (child == null) {
