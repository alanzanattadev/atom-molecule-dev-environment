          ...
          questionSystem
            ...
            .then(answers => {
              if (answers === null) {
                ...
                observer.next({
                  strategy: {
                    ...prepareStrategy(strategy, shell),
                    env,
                    shell,
                  },
                  controller,
...
function prepareStrategy(strategy: Strategy, shell: string): Strategy {
  if (shell == "bash.exe -c") {
    strategy = {
      ...strategy,
      command: convertPathForBash(strategy.command),
    };
  }
  return strategy;
}

function convertPathForBash(path: string) {
  return path
    .split("\\")
    .join("/")
    .replace(/([A-Z]):/, (x, p1) => {
...
export const RunStrategy = ({
  ...
  Rx.Observable.create(observer => {
    ...
    const filesWatcher = controller.watchFiles
      ? controller.watchFiles({
          toolPackages: selectPackagesOfTool(
            selectPackagesReducer(getState()),
            toolID,
          ),
        })
      : undefined;
    ...
    let fileObservables = undefined;
    if (
      filesWatcher &&
      filesWatcher.settings &&
      filesWatcher.settings.paths &&
      filesWatcher.settings.paths instanceof Array
    ) {
      fileObservables = filesWatcher.settings.paths.map(path =>
        createFilesWatcherObservable(
          watchPath,
          path,
          filesWatcher.settings.fileFilter || /(.)*$/gi,
        ).subscribe(x => filesWatcher.onFilesChanged(x, fileEventsHelpers)),
      );
    }
    let execution = {
      task,
    ...
    return function unsubscribe() {
      child.kill();
      if (fileObservables) {
        fileObservables.forEach(obs => obs.unsubscribe());
      }
    };
  });
    ...
    const child = exec(
      {
        onStart(child) {
          ...
          handleResize = ({ cols, rows }) => child.resize(cols, rows);
...
export default (context: ExecutionContext) => (action$, store) =>
  action$.ofType(RUN_TASK).mergeMap((action: RunTaskAction) => {
    ...
    return Rx.Observable.zip(strategy$, stager$, (strategy, stager) => ({
      strategy,
      stager,
    }))
      .mergeMap(({ strategy: { strategy, controller }, stager }) => {
        ...
        function getRunStrategy() {
          return RunStrategy(context)({
            ...
            taskID,
            stager,
            toolID: action.payload.plan.tool.id,
          });
        }
