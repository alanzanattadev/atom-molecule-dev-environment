...
import type {Directory as PackageDirectory, Package, Plugin} from "../Types/types.js.flow";
...
export function findPackages(
  ...
  let getPackageOfEntry = entry => plugins.map(plugin => {
      ...
      entry =>
        entry.isDirectory()
          ? [
              ...
              getPackageOfEntry(entry),
            ]
          : [getPackageOfEntry(entry)],
    )
    .reduce((red, value, i) => red.concat([].concat.apply([], value)), List())
    .filter(v => v !== false)
    .toJS();
}
