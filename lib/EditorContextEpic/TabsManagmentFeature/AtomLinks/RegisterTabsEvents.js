...
export function observeTabs() {
  atom.workspace.onDidAddPaneItem(({item, pane}) => {
    getAssociatedFiles(item.getPath()).filter(filePath => new File(filePath).existsSync()).forEach(filePath => {
      process.nextTick(function() {
        atom.workspace.open(filePath, {split: 'right', searchAllPanes: true});
      })
    })
  });
  ...
  atom.workspace.onDidDestroyPaneItem(({item, pane}) => {
    ...
    atom.workspace.getPaneItems().filter(paneItem => paneItem.getPath() != item.getPath()).forEach(paneItem => {
