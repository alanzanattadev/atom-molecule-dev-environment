...
export function stashPop(packageInfos: PackageInfos): StashPopAction {
  return () => {
    exec("git stash pop", { cwd: path.dirname(packageInfos.path) }, (
      err,
      stdout,
      stderr,
    ) => {
      if (err) {
        global.atom.notifications.addError("Impossible to stash pop", {
          detail: err + "\n" + stderr,
        });
      } else {
        global.atom.notifications.addSuccess("stash popped", {
          detail: stdout,
        });
      }
    });
