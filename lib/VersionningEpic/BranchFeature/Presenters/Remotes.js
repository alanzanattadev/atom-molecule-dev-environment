...
export class Remotes extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <ul style={{padding: '0px', display: 'flex', flex: '1', margin: '5px', justifyContent: 'center', alignItems: 'center'}}>
        {this.props.remotes.map(remote => (
          <li style={{listStyle: 'none', flex: '1', display: 'flex',}} key={remote.name}>
