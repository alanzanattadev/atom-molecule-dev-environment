...
export function pullBranch(remote: string, branch: string, packageInfos: PackageInfos): PullBranchAction {
  return dispatch => {
    let child = exec(
      ...
      (err, stdout, stderr) => {
        if (err) {
          atom.notifications.addError(`Impossible to pull ${remote} ${branch}`, {
            detail: stderr + '\n' + err.toString()
          });
        } else {
