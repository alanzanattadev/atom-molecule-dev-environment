...
export default class SelectableFilePaths extends React.Component<DefaultProps, Props, State> {
      ...
      <ul style={{padding: '0px'}}>
        {this.props.files.map(file => (
          <li style={{listStyle: 'none'}} key={file.path}>
            <SelectableFilePath {...file} onChange={(value: boolean) => this.props.onFileChange(file, value)}/>
          </li>
        ))}
      </ul>
    )
