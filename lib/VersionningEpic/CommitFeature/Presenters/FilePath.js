...
export default class FilePath extends React.Component<DefaultProps, Props, State> {
  ...
  render() {
    return (
      <span style={{fontSize: '18px', fontFamily: 'Inconsolata', color: this.getColor(this.props.status), whiteSpace: 'nowrap', textAlign: 'right', textOverflow: 'ellipsis'}}>{this.props.path}</span>
    )
  }
}
