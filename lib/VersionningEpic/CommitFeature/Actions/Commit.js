...
export function commit(message: string, files: Array<File>, packageInfos: PackageInfos): CommitAction {
  return dispatch => {
    let child = exec(
      `git add ${files.filter(file => file.status != 'removed').map(file => file.path).join(' ')} && git commit -m "${message}"`,
