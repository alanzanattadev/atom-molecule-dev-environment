...
import {renderVersionningPanel} from "../../VersionningEpic/CommitFeature/AtomLinks/Panels";
import {renderDevtoolBar, devtoolBarPanel} from '../../ExecutionControlEpic/DevToolsSummaryFeature/AtomLinks/Panels';
  ...
  ReactDOM.render((
    <Provider store={store}>
      <StyleRoot style={{flex: '1', alignItems: 'stretch', display: 'flex'}}>
        <SideDock onVersionning={() => {
          renderVersionningPanel(sidePanel);
          sidePanelPanel.show();
        }} onTargetConfigurer={() => {
          sidePanelPanel.show();
          if (devtoolBarPanel.isVisible()) {
...
export var sidePanelPanel = atom.workspace.addRightPanel({
  item: sidePanel,
  visible: false,
  priority: 50
})
